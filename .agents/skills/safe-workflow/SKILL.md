...
> **TEMPLATE**: This skill uses `XAN` as a placeholder. Replace with your project's ticket prefix (e.g., `WOR`, `PROJ`, `FEAT`).
...
**Format**: `XAN-{number}-{short-description}`

```text
# Good
XAN-447-create-safe-workflow-skill
XAN-123-fix-login-redirect
...
**Format**: `type(scope): description [XAN-XXX]`
...
```text
feat(harness): create safe-workflow skill [XAN-447]
fix(auth): resolve login redirect [XAN-57]
```
...
git checkout -b XAN-{number}-{description}

# 3. Make commits
git commit -m "type(scope): description [XAN-XXX]"
...
4. CI passes: `cd backend; go test ./...; go vet ./...; go build ./...; cd ../web; npm run lint; npm run build`
