...
CREATE POLICY user_data_user_select ON user_data
  FOR SELECT TO Xanders AI Video Platform_app_user
...
-- Grant permissions
GRANT SELECT, INSERT, UPDATE ON user_data TO Xanders AI Video Platform_app_user;
```
...
```bash
# Test migration
Run the configured Goose migration command for the target environment
...
CREATE POLICY {table}_user_insert ON {table}
  FOR INSERT TO Xanders AI Video Platform_app_user
...
CREATE POLICY {table}_admin_all ON {table}
  FOR ALL TO Xanders AI Video Platform_app_user
