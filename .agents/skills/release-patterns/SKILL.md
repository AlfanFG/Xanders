...
gh pr create --title "feat: add feature"  # Missing [XAN-XXX]
...
git push origin feature  # Without cd backend; go test ./...; go vet ./...; go build ./...; cd ../web; npm run lint; npm run build first
...
gh pr create --title "feat(scope): description [XAN-XXX]"

# CORRECT: Rebase merge only
gh pr merge --rebase --delete-branch

# CORRECT: CI validation before push
cd backend; go test ./...; go vet ./...; go build ./...; cd ../web; npm run lint; npm run build && git push --force-with-lease

# CORRECT: Always rebase first
git fetch origin && git rebase origin/main
git push --force-with-lease origin XAN-XXX-description
```
...
- [ ] Branch name: `XAN-{number}-{description}`
- [ ] Commits follow: `type(scope): description [XAN-XXX]`
- [ ] Rebased on latest main: `git fetch origin && git rebase origin/main`
- [ ] CI passes locally: `cd backend; go test ./...; go vet ./...; go build ./...; cd ../web; npm run lint; npm run build`
...
cd backend; go test ./...; go vet ./...; go build ./...; cd ../web; npm run lint; npm run build && echo "READY FOR PR" || echo "FIX ISSUES FIRST"
```

## PR Creation Template

```bash
gh pr create --title "feat(scope): description [XAN-XXX]" --body "$(cat <<'EOF'
## Summary

Implements [feature/fix] as specified in ticket XAN-XXX.

**Ticket**: https://linear.app/{{LINEAR_WORKSPACE}}/issue/XAN-XXX
...
cd backend; go test ./...; go vet ./...; go build ./...; cd ../web; npm run lint; npm run build
# All checks passed
...
```text
Prompt: "Review PR #XXX for XAN-YYY. Validate commit format,
CI status, patterns."
```
...
- feat(scope): description [XAN-XXX]
- fix(scope): description [XAN-YYY]
